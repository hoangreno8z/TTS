"""Google Colab Reproducible Benchmark Script for GPT-SoVITS (Vietnamese).
...
def run_gpt_sovits_synthesis(ref_audio_path="ref_sample.wav", ref_text=""):
    os.makedirs("output_gpt_sovits", exist_ok=True)
    from GPT_SoVITS.inference_webui import get_tts_wav

    print("\n--- Starting GPT-SoVITS 20 Benchmark Sentences ---")
    for s in BENCHMARK_SENTENCES:
        s_id = s["id"]
        text = s["text"]
        out_file = f"output_gpt_sovits/sovits_bench_{s_id:02d}.wav"
        t0 = time.time()
    ...
    print("\nGPT-SoVITS Benchmark generation completed! Run '!zip -r gpt_sovits_benchmark.zip output_gpt_sovits' to download.")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        run_gpt_sovits_synthesis(sys.argv[1])
    else:
